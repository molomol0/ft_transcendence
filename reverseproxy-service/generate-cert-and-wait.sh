...
if [ ! -f /etc/nginx/ssl/certificate.crt ]; then
    ...
    openssl req -x509 -newkey rsa:4096 -keyout /etc/nginx/ssl/private.key -out /etc/nginx/ssl/certificate.crt -days 365 -nodes -subj "/C=US/ST=State/L=City/O=Company/CN=sxpongo.ddns.net"
